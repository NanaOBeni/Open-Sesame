export async function get_country_url(country: string): Promise<string> {
    ...
    console.log('api key:', import.meta.env.VITE_GEOAPIFY_KEY);
    ...
    console.log('boundary response:', boundary_json);
    const randomFeature = boundary_json.features[Math.floor(Math.random() * boundary_json.features.length)];
    console.log('random feature geometry:', randomFeature.geometry);
    const coords = randomFeature.geometry.coordinates[0];
